...
interface DiaryPostClientProps {
    postId: string;
    initialPost: any; // Using any for serialized data
    initialAuthor: any;
}

export default function DiaryPostClient({ postId, initialPost, initialAuthor }: DiaryPostClientProps) {
    const { user } = useDiaryAuth();
    const [post, setPost] = useState<DiaryPost | null>(initialPost);
    const [author, setAuthor] = useState<DiaryProfile | null>(initialAuthor);
    ...
    const loadRelatedData = async () => {
        try {
            if (post) {
                const [authorPosts, explore] = await Promise.all([
                    getPostsByAuthor(post.authorId, 'published'),
                    getPublishedPosts(3),
                ]);
                setMoreFromAuthor(authorPosts.filter(p => p.postId !== postId).slice(0, 3));
                setExplorePosts(explore.posts.filter(p => p.postId !== postId && p.authorId !== post.authorId).slice(0, 3));
            }
        } catch (err) {

...
export default function BasketballAdminLayout({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        { name: 'Dashboard', href: '/admin/basketball', icon: LayoutDashboard },
    ...
    return (
        <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
            ...
            <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 z-40">
                <div className="font-display font-bold text-orange-600">Basketball</div>
                <button
            ...
            `}>
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 hidden md:block">
                    <h1 className="text-2xl font-display font-bold text-orange-600 flex items-center gap-2">
                        <span className="text-3xl">🏀</span> Basketball
                    </h1>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    ...
                    {navItems.map((item) => {
                        const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== '/admin/basketball');
                        return (
                            <Link
                                key={item.name}
                                href={item.href}
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-3 px-4 py-3">
                        <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center">
                            <span className="text-xs font-bold text-orange-600">AD</span>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-black dark:text-white">Admin User</p>
                            <p className="text-xs text-black dark:text-gray-400">admin@ppsachronicles.com</p>
                        </div>
                    </div>
                </div>
            </aside>
